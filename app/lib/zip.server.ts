...
//
// Store only, no deflate. Deliverables are PDFs, images, and video stills, all of
// them already compressed, so level 6 bought a percent or two of size for a
// synchronous compression pass over every byte of the archive. That pass is what put
// a large selection over the Worker CPU limit, and Cloudflare answers that with a
// 503 rather than anything the organizer can act on.
...
/** Store-only archive. Duplicate paths get a numeric suffix. */
