...
const contacts = [
  ...
  { id: 3, email: "speaker@demo.meridian.dev", first: "Elena", last: "Sorescu", title: "Engineering Manager", company: "Halcyon Data", bio: "Elena manages the data platform group at Halcyon Data and speaks regularly on schema evolution and streaming systems.", status: "confirmed", dietary: "Vegetarian", tshirt: "M", travel: "Arrives June 9 evening, departs June 12. Aisle seat." },
  { id: 4, email: "tomas.lindgren@meridian.demo", first: "Tomas", last: "Lindgren", title: "SRE Lead", company: "Nordwind", bio: "Tomas runs reliability for Nordwind's edge network.", status: "confirmed", tshirt: "L" },
  { id: 5, email: "aisha.bello@meridian.demo", first: "Aisha", last: "Bello", title: "Security Engineer", company: "Kestrel Security", bio: "Aisha works on supply-chain security tooling and SBOM automation.", status: "invited" },
  ...
  { id: 7, email: "mira.house@meridian.demo", first: "Mira", last: "House", title: "Product Engineer", company: "Loamworks", bio: "Mira ships developer onboarding tooling and measures everything.", status: "confirmed" },
  { id: 8, email: "diego.paz@meridian.demo", first: "Diego", last: "Paz", title: "Platform Architect", company: "Sable Cloud", bio: "Diego designs multi-region control planes and writes about platform migrations.", status: "confirmed" },
];
...
const main = async () => {
  ...
  const tables = [
    "users","events","contacts","event_contacts","tracks","formats","levels","tags","rooms","statuses","forms","form_fields","sessions","session_participants","session_tags","eval_plans","eval_criteria","eval_plan_reviewers","eval_assignments","eval_scores","ai_reviews","portal_tasks","task_assignees","task_completions","file_requests","file_request_assignees","file_uploads","file_comments","blobs","email_templates","email_sends","jobs","airtable_links","api_tokens","settings",
  ];
  ...
  push(`INSERT INTO eval_plan_reviewers (id,plan_id,user_id,created_at) VALUES (1,1,6,${NOW});`);
  const assignments = [
  ...
  const tasks = [
    ...
    ["Book travel", "Send us your arrival and departure so we can plan transfers.", "2027-05-15T23:59:00Z", "accepted_speakers"],
    ["Sign speaker release form", "Required before we can record your session.", "2027-05-20T23:59:00Z", "selected"],
  ];
  tasks.forEach((t, i) =>
  ...
  // The release form is assigned to two named speakers, so "selected" has real data.
  [3, 4].forEach((contactId) => push(`INSERT INTO task_assignees (task_id,contact_id) VALUES (4,${contactId});`));
  // Mixed completion, so the organizer matrix is not all one colour.
  const completions = [[1, 3], [2, 3], [1, 4]];
  completions.forEach(([taskId, contactId]) =>
    push(`INSERT INTO task_completions (task_id,contact_id,status,completed_at) VALUES (${taskId},${contactId},'done',${NOW});`)
  );

  // File requests, with a full review cycle behind them: a denied v1 that came back
  // as an approved v2, and one deliverable still waiting for review.
  const requests = [
    [1, "Final slides", "Upload your final slide deck as PDF, 16:9 aspect ratio.", "2027-06-01T23:59:00Z", "accepted_speakers"],
    [2, "Speaker headshot (print quality)", "Square JPEG or PNG, at least 1200px, no logo overlay.", "2027-05-20T23:59:00Z", "accepted_speakers"],
  ];
  requests.forEach(([id, title, instructions, due, applies]) =>
    push(
      `INSERT INTO file_requests (id,event_id,title,instructions,due_at,applies_to,created_at) VALUES (${id},${EV},${q(title)},${q(instructions)},${ts(due)},${q(applies)},${NOW});`
    )
  );

  // Tiny but valid files so downloads, previews, and the ZIP export all work.
  ...
  // [uploadId, requestId, contactId, sessionId, version, blobKey, filename, type, bytes, approval, uploaderUserId, ageHours]
  const uploads = [
  ...
  for (const [id, requestId, contactId, sessionId, version, key, filename, type, bytes, approval, uploader, ageHours] of uploads) {
    push(
      `INSERT INTO blobs (key,data,content_type,size,created_at) VALUES (${q(key)},${hex(bytes)},${q(type)},${bytes.length},${NOW - ageHours * 3600});`
    );
    const reviewed = approval === "pending" ? "NULL" : String(NOW - (ageHours - 2) * 3600);
    push(
  ...
  const comments = [
    [1, 7, "Draft deck, final version coming Friday.", 95],
    [1, 5, "Slide 12 is still 4:3 and the code samples clip. Re-export at 16:9 and upload a new version.", 90],
    [2, 7, "Re-exported at 16:9 and fixed the clipping on slide 12.", 24],
    [2, 5, "Approved. Thanks for the quick turnaround.", 22],
  ];
  comments.forEach(([uploadId, author, body, ageHours]) =>
    push(
      `INSERT INTO file_comments (upload_id,author_user_id,body,created_at) VALUES (${uploadId},${author},${q(body)},${NOW - ageHours * 3600});`
    )
  );
  ...
  const templates = [
    ...
    ["decline", "Decline", "Update on your {event_name} proposal", "<p>Hi {speaker_name},</p><p>Thank you for submitting \"{talk_title}\" to {event_name}. We are not able to include it this year. We had far more strong proposals than slots, and we would still love to see you at the event.</p>{#feedback}<p>{feedback}</p>{/feedback}"],
